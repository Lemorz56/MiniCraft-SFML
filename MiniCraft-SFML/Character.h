...
class Character : public sf::Drawable
   //publically inheriting from "Drawable" class for cleaner syntax when drawing to screen
{
private:
   ...
   {
      down = 0,
      //row zero on sprite sheet
      left = 1,
      //row one on sprite sheet
      up = 3,
      //row two on sprite sheet
      right = 2,
      //row three on sprite sheet
   ...
   Facing m_dir;
   //dir represents not only direction but also the "row" on the sprite shee that is being used for animation
...
public:
   Character(const std::string& filename, int sprW, int sprH, int nFrames, bool eightDir = false,
             bool slopingMovement = false);
   ~Character() override;
