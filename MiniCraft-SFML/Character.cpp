...
Character::Character(std::string filename, int sprW, int sprH, int nFrames, bool eightDirs, bool slopeMove)
{
    ...
    m_eightDirections = eightDirs;//bool value, if false 4 directional animation, if true, 8 directional animation, defaults to 4
    m_slopingMovement = slopeMove;//bool balue, if false, character speeds are static, if true, character speed "slopes" to a limit, then slopes back down to zero upon key release
...
void Character::checkCollision(WorldObject& obj) {
    m_collision = false;//defaullt collision to false
    m_collisionBox = _character.getGlobalBounds();
    if (m_collisionBox.intersects(obj.getCollisionBox())) {//update collision
        m_collision = true;
...
void Character::charMoveStatic()
{
    ...
    if (m_eightDirections) {//if using 8 directional sprite sheet
...
void Character::animate(bool isMoving)
{
    ...
    if (m_currentFrameTime > m_nextFrameTime) {//execute if the current frame time has surpassed the next frame's expected display time
        m_nextFrameTime = m_currentFrameTime + m_timeToWait;
        if (isMoving) {//if player is currently moving, anime sprite
            m_animFrameX = m_animFrameX + m_spriteWidth;//increments frame value to next frame in animation, but this value is only used when it is time for next frame to be displayed
            if (m_animFrameX >= (m_spriteWidth * m_numFrames)) { m_animFrameX = 0; }//resets frame to starting point if last frame in sequence has been displayed
        }
        else {//if player is NOT moving, reset to first frame
            m_animFrameX = 0;
        }
        _character.setTextureRect(sf::IntRect(m_animFrameX, m_dir * m_spriteHeight, m_spriteWidth, m_spriteHeight));//finds appropriate "frame" in tileset, assigns to sprite
    }
